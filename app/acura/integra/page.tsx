...
export default function AcuraIntegraPage() {
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid')
  const [searchTerm, setSearchTerm] = useState('')

  const integraProducts = useMemo(() => {
    return acuraProducts.filter(p => p.name.includes('Integra'))
  }, [])

  const filteredProducts = useMemo(() => {
    return integraProducts.filter(p =>
      p.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      p.description.toLowerCase().includes(searchTerm.toLowerCase())
    )
  }, [searchTerm, integraProducts])
      ...
      <main>
        <section className="py-12 bg-background border-b border-border/20">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <h1 className="text-4xl sm:text-5xl font-black text-foreground mb-4">Acura Integra Parts</h1>
            <p className="text-lg text-muted-foreground max-w-2xl mb-8">
              Browse {integraProducts.length} quality used parts for Acura Integra models (1986-2001).
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  type="text"
                  placeholder="Search Integra parts..."
                  value={searchTerm}
        ...
        <PartsDetails partType="Acura Integra" yearRange="1986-2001" mileageRange="0-200,000 miles" />
        <AcuraModelHistoryIndex modelSlug="integra" products={integraProducts} />
        <ShippingInfo />
        <ProductFAQ productType="Acura Integra Parts" />
      </main>
      <Footer />
    </>
  )

...
export default function AcuraMdxPage() {
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid')
  const [searchTerm, setSearchTerm] = useState('')

  const mdxProducts = useMemo(() => {
    return (acuraProducts || []).filter(p => p.name.includes('MDX'))
  }, [])

  const filteredProducts = useMemo(() => {
    return mdxProducts.filter(p =>
      p.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      p.description.toLowerCase().includes(searchTerm.toLowerCase())
    )
  }, [searchTerm, mdxProducts])
      ...
      <main>
        <section className="py-12 bg-background border-b border-border/20">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <h1 className="text-4xl sm:text-5xl font-black text-foreground mb-4">Acura MDX Parts</h1>
            <p className="text-lg text-muted-foreground max-w-2xl mb-8">
              Browse {mdxProducts.length} quality used parts for Acura MDX SUV models with guaranteed pricing.
            </p>
        ...
        <PartsDetails partType="Acura MDX" yearRange="2001-Present" mileageRange="0-200,000 miles" />
        <AcuraModelHistoryIndex modelSlug="mdx" products={mdxProducts} />
        <ShippingInfo />
        <ProductFAQ productType="Acura MDX Parts" />
      </main>
      <Footer />
    </>
  )

...
export default function AcuraClPage() {
  ...
  const clProducts = useMemo(() => {
    return acuraProducts.filter(p => p.name.includes('CL') && !p.name.includes('CSX'))
  }, [])

  const filteredProducts = useMemo(() => {
    return clProducts.filter(p =>
      p.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      p.description.toLowerCase().includes(searchTerm.toLowerCase())
    )
  }, [searchTerm, clProducts])
      ...
      <main>
        <section className="py-12 bg-background border-b border-border/20">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <h1 className="text-4xl sm:text-5xl font-black text-foreground mb-4">Acura CL Parts</h1>
            <p className="text-lg text-muted-foreground max-w-2xl mb-8">
              Browse {clProducts.length} quality used parts for Acura CL models with guaranteed pricing.
            </p>
        ...
        <PartsDetails partType="Acura CL" yearRange="1997-2003" mileageRange="0-200,000 miles" />
        <AcuraModelHistoryIndex modelSlug="cl" products={clProducts} />
        <ShippingInfo />
        <ProductFAQ productType="Acura CL Parts" />
      </main>
      <Footer />
    </>
  )
